...
public class MultiTouchActivity extends BaseActivity implements OnTouchListener {
	...
	private static final String MULTI_TOUCH_TAG = "MultiTouchActivity";

	@Override
	public String getTag() {
		return MULTI_TOUCH_TAG;
	}
	
	private View parent;

	@SuppressWarnings("unchecked")
	private final ArrayList<View>[] recentTouchedViewsIndex = (ArrayList<View>[]) new ArrayList[10];

	@SuppressWarnings("unchecked")
	private final ArrayList<View>[] downTouchedViewsIndex = (ArrayList<View>[]) new ArrayList[10];
		...
		for (final View view : hoverViews) {
			...
			if (recentTouchedViewsIndex[actionPointerID] != null) {
				if (pointInView(x, y, mTouchSlop, view.getWidth(),
						view.getHeight())) {
					// DebugLog.v("tag", "added because < mTouchSlop");
	...
	private void dumpEvent(MotionEvent event) {
	   ...
	   if (actionCode == MotionEvent.ACTION_POINTER_DOWN
	      ...
	      event.getPointerId(action >> MotionEvent.ACTION_POINTER_INDEX_SHIFT));
	      sb.append(")" );
	   }
	   sb.append("[" );
